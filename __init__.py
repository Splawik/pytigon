    ...
    for object in Scripts.objects.all():
        if object.menu:
            ...
            if len(elements)>2:
                if elements[0]=='main':
                    if len(elements)>3:
                        ret.append(('/simplescripts/run/'+object.name, elements[1], None, elements[2]))
                    else:
                        ret.append(('/simplescripts/run/'+object.name, elements[1], None, 'client://apps/utilities-terminal.png'))
    return ret
