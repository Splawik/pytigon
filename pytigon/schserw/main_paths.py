...
def get_main_paths():
    ...
    if not ret["SERW_PATH"] in sys.path:
        sys.path.append(ret["SERW_PATH"])
    if not ret["ROOT_PATH"] in sys.path:
    ...
    if platform_name() == "Android" or "PYTIGON_APP_IMAGE" in environ:
        ...
        if 'START_PATH' in environ:
            ...
            cwd_prj = os.path.join(os.path.abspath(os.getcwd()), "prj")
        if os.path.exists(cwd_prj):
            ret["PRJ_PATH"] = cwd_prj
            ret["PRJ_PATH_ALT"] = os.path.join(ret["ROOT_PATH"], "prj")
        else:
            ret["PRJ_PATH"] = os.path.join(ret["DATA_PATH"], "prj")
            ret["PRJ_PATH_ALT"] = os.path.join(ret["ROOT_PATH"], "prj")
    return ret
