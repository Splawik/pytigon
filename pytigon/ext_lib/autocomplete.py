...
class TextCtrlAutoComplete(wx.TextCtrl, listmix.ColumnSorterMixin):
    def __init__(
        ...
        **therest
        ...
        self.Bind(wx.EVT_WINDOW_DESTROY, self.handler_close)
...
class test:
    def __init__(self):
        ...
        app = wx.PySimpleApp()
        frm = wx.Frame(
            None, -1, "Test", style=wx.TAB_TRAVERSAL | wx.DEFAULT_FRAME_STYLE
        )
        panel = wx.Panel(frm)
    ...
    def onBtMultiChoice(self, event):
        # change the choices
        self._ctrl.SetMultipleChoices(
            [("Test", "Hello"), ("Other word", "World"), ("Yes!", "it work?")],
            colFetch=1,
        )
        self._ctrl.SetEntryCallback(None)
        self._ctrl.SetMatchFunction(None)
