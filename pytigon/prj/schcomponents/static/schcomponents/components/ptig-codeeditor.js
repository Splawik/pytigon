...
export var TEMPLATE = '<nav class=\"navbar navbar-light bg-light\">\n' +
    ...
    '                        <slot></slot>\n' +
    '                </div>\n' +
    '                <button data-bind=\"disabled:!changed;onclick:on_save\" class=\"btn btn-primary col-sm-2 col-md-2 col-lg-2\" type=\"button\">\n' +
...
var comp = DefineWebComponent (TAG);
try {
	comp.__enter__ ();
	...
	var init = comp.fun ('init') (function (component) {
		var _on_loadjs = function () {
			var ed = component.querySelector ('div.vseditor');
			...
			var _on_resize = function (event) {
				var parent_dy = ed.parentElement.offsetHeight;
				var y = ed.offsetTop;
				if (parent_dy - y > 0) {
