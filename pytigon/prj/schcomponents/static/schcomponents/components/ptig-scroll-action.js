// Transcrypt'ed from Python, 2020-09-29 22:47:41
	...
	var init = comp.fun ('init') (function (component) {
		...
		var handle_scroll = function (event) {
			...
			if (component.hasAttribute ('y')) {
				var window_y = int ((int (component.getAttribute ('y').py_replace ('%', '')) * window_dy) / 100);
			}
			else {
			...
			var v = (2 * ((div_y1 + div_y2) / 2 - window_y)) / active_zone_dy;
			print ((div_y1 + div_y2) / 2, window_y, active_zone_dy, component.getAttribute ('y'), window_dy);
