...
class WikiGit():
    def __init__(self, wikiconf_item):
        ...
        git_base_path = os.path.join(settings.DATA_PATH, settings.PRJ_NAME, 'git_repositories')
    ...
    def wiki_pull(self, erase_previous_data=False):
        self._pull()
        ...
        for root, dirs, files in os.walk(self.git_subject_path):
            if not ".git" in root.replace("\\", "/").split("/"):
                ...
                for file in files:
                    ...
                    if file[2]=='.' and file[0] >= '0' and file[0] <= '9' and file[1] >= '0' and file[1] <= '9': 
                        menu_position = int(file[:2])
                        description = file.split('.', 1)[1].strip().replace(".imd", "").replace(".md", "")
                    else:
                        menu_position = None
                        description = file.replace(".imd", "").replace(".md", "")
                    
                    name = wiki_from_str(description)
                    old_page = Page.objects.filter(subject=self.wikiconf_item.subject, published=True, name=name).first()
                    if old_page:
    ...
    def wiki_push(self):
        if not os.path.exists(self.git_path):
            self._pull()
        conf = {}
        objects_list = Page.objects.filter(subject=self.wikiconf_item.subject, published=True)
        for obj in objects_list:
            if obj.content_src:
                ...
                if file_name:
                    if '/' in file_name:
                        file_path = os.path.join(self.git_subject_path, file_name.rsplit('/',1)[0])
                    else:
