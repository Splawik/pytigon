...
window.REINIT = null;
export var app_reinit = function() {
  if (window.REINIT) {
    return app_init(window.REINIT[0], window.REINIT[1], window.REINIT[2], window.REINIT[3], window.REINIT[4], window.REINIT[5], window.REINIT[6], window.REINIT[7], window.REINIT[8], window.REINIT[9]);
  }
};
...
export var app_init = function(prj_name, application_template, menu_id, lang, base_path, base_fragment_init, component_init, offline_support, start_page, gen_time, callback) {
  ...
  if (jQuery("#dialog-form-modal").length == 0) {
    window.REINIT = [prj_name, application_template, menu_id, lang, base_path, base_fragment_init, component_init, offline_support, start_page, gen_time];
    var _on_login_submit = function(e) {
      ...
      var _on_login_submit2 = function(data) {
        ...
        if (start > 0 && end > 0) {
          var title = jQuery("<div>" + data + "</div>").find("title").text();
          var data = data.substring(start + 6, end - 1);
          var data = "<title>" + title + "</title>" + data + "";
        }
...
export var on_new_tab = function(url, elem, e) {
...
export var EVENT_TAB = [tuple(["*", "get_tbl_value", true, false, on_get_tbl_value]), tuple(["*", "new_tbl_value", true, false, on_new_tbl_value]), tuple(["*", "get_row", true, false, on_get_row]), tuple(["popup_edit", "*", true, false, on_popup_edit_new]), tuple(["popup_info", "*", true, false, on_popup_info]), tuple(["popup_delete", "*", true, false, on_popup_delete]), tuple(["inline", "*", true, false, on_popup_inline]), tuple(["_top", "*", true, false, on_new_tab]), tuple(["_top2", "*", true, 
false, on_new_tab]), tuple(["refresh_obj", "*", true, false, refresh_current_object]), tuple(["refresh_page", "*", true, false, refresh_current_page]), tuple(["refresh_app", "*", false, false, refresh_current_app]), tuple(["run_script", "*", false, false, get_and_run_script]), tuple(["null", "*", false, false, only_get])];
export var standard_on_data = function(src_obj, href) {
