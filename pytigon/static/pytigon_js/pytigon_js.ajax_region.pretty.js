...
import{init_table}from "./pytigon_js.tbl.js";
import{Loading, ajax_get, ajax_post, corect_href, get_table_type, super_insert}from "./pytigon_js.tools.js";
var __name__ = "pytigon_js.ajax_region";
export var MOUNT_INIT_FUN = [];
export var register_mount_fun = function(fun) {
  MOUNT_INIT_FUN.append(fun);
};
export var mount_html = function(dest_elem, data_or_html) {
  var _on_remove = function(index, value) {
    value.on_remove();
  };
  jQuery.each(jQuery(dest_elem).find(".call_on_remove"), _on_remove);
  if (py_typeof(data_or_html) == str) {
...
export var datetime_init = function(dest_elem) {
  ...
  var d = jQuery(dest_elem).find("div.form-group .datefield input");
  d.daterangepicker(format);
  format["locale"]["format"] = "YYYY-MM-DD HH:mm";
  format["timePicker"] = true;
  format["timePickerIncrement"] = 30;
  var d = jQuery(dest_elem).find("div.form-group .datetimefield input");
  d.daterangepicker(format);
};
register_mount_fun(datetime_init);
export var selectpicker_init = function(dest_elem) {
  jQuery(dest_elem).find(".selectpicker").selectpicker();
};
register_mount_fun(selectpicker_init);
export var moveelement_init = function(dest_elem) {
  var objs = dest_elem.querySelectorAll(".move-element");
  if (objs) {
    for (var obj of objs) {
...
export var label_floating_init = function(dest_elem) {
  ...
  jQuery(dest_elem).find(".label-floating .form-control").on("focus blur", _on_blur).trigger("blur");
};
register_mount_fun(label_floating_init);
export var select2_init = function(dest_elem) {
  jQuery(dest_elem).find(".django-select2:not(.select2-full-width)").djangoSelect2(dict({"width":"calc(100% - 48px)", "minimumInputLength":1}));
  jQuery(dest_elem).find(".django-select2.select2-full-width").djangoSelect2(dict({"width":"calc(100%)", "minimumInputLength":1}));
...
export var datatable_init = function(dest_elem) {
  var table_type = get_table_type(jQuery(dest_elem));
  var tbl = jQuery(dest_elem).find(".tabsort");
  if (tbl.length > 0) {
...
export var get_ajax_region = function(element, region_name) {
  if (typeof region_name == "undefined" || region_name != null && region_name.hasOwnProperty("__kwargtrans__")) {
    var region_name = null;
  }
  if (element.classList.contains("ajax-region") && (!region_name || element.getAttribute("data-region") == region_name)) {
    return element;
  } else {
    if (region_name) {
      var ret = element.closest(".ajax-region");
      while (ret) {
        if (ret.getAttribute("data-region") == region_name) {
          return ret;
        }
        var ret = ret.closest(".ajax-region");
      }
      if (region_name) {
...
export var get_ajax_link = function(element, region_name) {
  if (typeof region_name == "undefined" || region_name != null && region_name.hasOwnProperty("__kwargtrans__")) {
    var region_name = null;
  }
  if (element.classList.contains("ajax-link") && (!region_name || element.getAttribute("data-region") == region_name)) {
    return element;
  }
  var region = get_ajax_region(element, region_name);
  if (region) {
    if (region.classList.contains("ajax-link")) {
      return region;
    } else {
      if (region_name) {
        var link = region.querySelector(".ajax-link[data-region='" + region_name + "']");
        if (link) {
          return link;
        }
      } else {
...
export var get_ajax_frame = function(element, region_name) {
  if (typeof region_name == "undefined" || region_name != null && region_name.hasOwnProperty("__kwargtrans__")) {
    var region_name = null;
  }
  var region = get_ajax_region(element, region_name);
  if (region) {
    if (region.classList.contains("ajax-frame") && (!region_name || region.getAttribute("data-region") == region_name)) {
      return region;
    } else {
      if (region_name) {
        var frame = region.querySelector(".ajax-frame[data-region='" + region_name + "']");
        if (frame) {
          return frame;
        }
      } else {
...
export var refresh_ajax_frame = function(element, region_name, data_element, callback) {
  if (typeof region_name == "undefined" || region_name != null && region_name.hasOwnProperty("__kwargtrans__")) {
    var region_name = null;
  }
  if (typeof data_element == "undefined" || data_element != null && data_element.hasOwnProperty("__kwargtrans__")) {
    var data_element = null;
  }
  if (typeof callback == "undefined" || callback != null && callback.hasOwnProperty("__kwargtrans__")) {
    var callback = null;
  }
  var frame = get_ajax_frame(element, region_name);
  if (!frame) {
    return;
  }
  var link = get_ajax_link(element, region_name);
  var loading = Loading(element);
  var _callback = function(data) {
    loading.stop();
    loading.remove();
    if (getattr(frame, "onloadeddata") && frame.onloadeddata) {
  ...
  if (link.hasAttribute("href")) {
    ...
    if (link.hasAttribute("action")) {
      var url = link.getAttribute("action");
      var post = true;
    } else {
  ...
  if (url) {
    ...
    loading.start();
    if (post) {
      var data = jQuery(link).serialize();
...
export var ajax_load = function(element, url, complete) {
...
export var clean_popups = function(element) {
  var elements = element.querySelectorAll(".plug");
