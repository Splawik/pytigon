...
function jsimp (module_path, callback = null) {
  if (callback) {
    let ret = import(module_path)
    ret.then(callback)
    return ret
  } else {
...
function make_callback (i, callback) {
  return function (module) {
    callback(i, module)
  }
}

function jsimp_many (modules, callback) {
  let count = 0,
    count2 = 0,
    results = []
  function callback_wrapper (index, module) {
    count++
    results[index] = module
    if (count === modules.length && callback) {
      callback(results)
    }
  }
  for (module_path of modules) {
    import(module_path).then(make_callback(count2, callback_wrapper))
    count2++
  }
}

window.jsimp_many = jsimp_many
...
function define_custom_element (tag, shadow, options) {
  class ProxyHTMLElement extends HTMLElement {
    ...
    handle_event (name, value, src, callback) {
      if (this.event_handler) {
        let component = this
        if (name in this.event_handler) {
          this.event_handler[name](component, value, src, callback)
        }
      }
    }
...
window.sync_on_unload = sync_on_unload

async function dynamic_import (url, callback) {
  import(url).then(callback)
}

window.dynamic_import = dynamic_import
...
window.getParamsFromUrl = getParamsFromUrl
...
window.getParamsFromEncodedParams = getParamsFromEncodedParams
...
function graphql_request () {
  ...
  if (token) {
    var g
    if (window.COMPONENT_INIT && 'graphql_prv' in window.COMPONENT_INIT) {
      g = window.COMPONENT_INIT.graphql_prv
    }
    else {
      g = window.BASE_PATH + "/graphql/"
    }

    ajax_post(url, data, complete, callback_on_error, process_req=None, content_type=None): 
    return graphql(g, {
      headers: {
...
window.graphql_request = graphql_request

function graphql_public_request () {
  if (window.COMPONENT_INIT && 'graphql_pub' in window.COMPONENT_INIT) {
    return graphql(window.COMPONENT_INIT.graphql_pub)
  } else {
    return graphql(window.BASE_PATH + "/graphql_public/")
  }
}

window.graphql_public_request = graphql_public_request
