...
# faces = {'times': 'Terminus', 'mono': 'Terminus', 'helv': 'Terminus', 'other':
# 'Terminus', 'size': 10, 'size2': 8}
    faces = {
        'times': 'Monospace',
        'mono': 'Monospace',
        'helv': 'Monospace',
...
# self.StyleSetSpec(stc.STC_STYLE_DEFAULT,"face:%s,size:%d" % (face, size))
    ...
    def set_ext(self, ext):
        if ext in EditorObjectMap:
            self.SetLanguage(EditorObjectMap[ext])

    def set_language(self, language):
        try:
            self.langObj = eval(language + 'EditorObject()')
        except:
            self.langObj = None
        if self.langObj:
            self.langObj.SetUpEditor(self)
        else:
...
class PythoneditorObject(object):
...
class HtmlLikePythoneditorObject(PythoneditorObject):

    def set_up_editor(self, editor):
        PythoneditorObject.SetUpEditor(self, editor)
...
class HtmleditorObject(object):

    def set_up_editor(self, editor):
        editor.SetKeyWords(0, htmlKeywords)


EditorObjectMap = {'py': 'PYTHON', 'html': 'HTML', 'phtml': 'HTML_LIKE_PYTHON'}
